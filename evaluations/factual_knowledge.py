...
def evaluate_model(model, tokenizer):
    with open("data/store-json/factual_prompts.json", "r") as f:
        prompts = json.load(f)
    ...
    for story in tqdm(prompts):
        ...
        # eval_msg = gemini_prompt(story_for_prompt,0)
        # with open("factual_knowledge_stories.txt", "a") as file:
        #     file.write(story_for_prompt)
        #     file.write("\n\n\n")
        # time.sleep(10)
        # evals = json.loads(eval_msg)

        # score += int(evals["factual knowledge"])
...
def main():
    ...
    with open("data/krct-results/factual_knowledge_stories.txt", "a") as file:
        file.write(eval_file)
        file.write('\n')
    ret = evaluate_model(model, tokenizer)
    print(ret)
    with open("eval3.txt", "a") as file:  
